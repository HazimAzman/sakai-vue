...
const loadDownloads = async () => {
    try {
        ...
        if (!downloads.value || downloads.value.length === 0) {
            downloads.value = [];
        }
    } catch (err) {
...
const getCleanUrl = (url) => {
    ...
    // Remove leading slash if it exists before a protocol
...
<template>
    <div id="download" class="py-6 px-6 lg:px-20 mx-0 my-12 lg:mx-20">
        ...
        <div v-else-if="error" class="text-center py-12">
            <div class="text-red-500 text-lg">Failed to load downloads. Please try again later.</div>
        </div>

        <div v-else class="bg-white dark:bg-surface-800 rounded-lg shadow-md overflow-hidden">
